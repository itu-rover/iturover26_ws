...
class ControlVCU:
    ...
    def get_feedback(self):
        ...
        if self.recv_msg.startswith("S") and self.recv_msg.endswith("X"):
            ...
            self.gnss[0] = struct.unpack(">i", bytes.fromhex(self.recv_msg[41:45]))[0]* (1e-7) #lat
            self.gnss[1] = struct.unpack(">i", bytes.fromhex(self.recv_msg[45:49]))[0]* (1e-7) #long
            self.gnss[2] = 0 #-int.from_bytes(data[21:23],byteorder='little') * (1e-4) #yaw
            self.gnss[3] = struct.unpack(">i", bytes.fromhex(self.recv_msg[49:53]))[0] * (1e-5) #cog 
            self.gnss[4] = struct.unpack(">i", bytes.fromhex(self.recv_msg[53:57]))[0]* (1e-3) #sog
            rospy.loginfo(f"{self.gnss[0]:.7f}     {self.gnss[1]:.7f}     {self.gnss[3]:.2f}     {self.gnss[4]:.2f}")
            ...
            gnss_pub.data = self.float
            self.pub_gnss.publish(gnss_pub)
        else:
            rospy.WARN("Received message from VCU is wrong.")
            rospy.WARN(f"Received message from VCU is: {self.recv_msg}")
